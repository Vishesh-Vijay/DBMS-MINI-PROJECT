        ...
        <div class="container my-5 px-5 mx-4">
            <table class="table-auto w-full scroll-ml-3">
                ...
                if(isset($_POST['submit'])){
                    $search=$_POST['search'];
                    $sql="Select * from `doctor` where Doctor_ID='$search'";
                    ...
                    if($result){
                        if(mysqli_num_rows($result) > 0){
                            ...
                            <th class="border px-4 py-2">Hospital ID</th> 
                            ...
                            echo'<tbody>
                            <tr>
                            <td class="border px-4 py-2">'.$row['Doctor_ID'].'</td>
                            <td class="border px-4 py-2">'.$row['first_name'].'</td>
                            <td class="border px-4 py-2">'.$row['last_name'].'</td>
                            <td class="border px-4 py-2">'.$row['department_name'].'</td>
                            <td class="border px-4 py-2">'.$row['hospital_id'].'</td>
